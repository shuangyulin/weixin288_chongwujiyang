...
<script>
	export default {
		...
		async onShow(options) {
			...
			let cleanType = uni.getStorageSync('discussdiscussjiyanghuanjingCleanType')
			if(cleanType){
				uni.removeStorageSync('discussdiscussjiyanghuanjingCleanType')
				this.mescroll.num = 1
		...
		methods: {
			// 支付
			onPayTap(){
				uni.setStorageSync('paytable','discussjiyanghuanjing');
			...
			onAcrossTap(tableName,crossOptAudit,statusColumnName,tips,statusColumnValue){
				uni.setStorageSync('crossTable','discussjiyanghuanjing');
			...
			// 获取详情
			async init(){
				let res = await this.$api.info('discussjiyanghuanjing', this.id);
			...
			async upCallback(mescroll) {
                let res = await this.$api.list('discussdiscussjiyanghuanjing', {
					page: mescroll.num,
					limit: 10,
					refid: this.id
				});
			...
			// 添加评论
			async onCommentTap() {
				this.$utils.jump(`../discussdiscussjiyanghuanjing/add-or-update?refid=${this.id}`)
			},
			onSHTap() {
			...
			async onFinishTap() {
				...
				await this.$api.update('discussjiyanghuanjing', this.detail);
				this.$utils.msg('审核成功');
