...
<script>
	export default {
		...
		methods: {
			// 支付
			onPayTap(){
				uni.setStorageSync('paytable','chongzhu');
			...
			onAcrossTap(tableName,crossOptAudit,statusColumnName,tips,statusColumnValue){
				uni.setStorageSync('crossTable','chongzhu');
			...
			// 获取详情
			async init(){
				let res = await this.$api.info('chongzhu', this.id);
			...
			// 添加评论
			async onCommentTap() {
				this.$utils.jump(`../discusschongzhu/add-or-update?refid=${this.id}`)
			},
			onSHTap() {
			...
			async onFinishTap() {
				...
				await this.$api.update('chongzhu', this.detail);
				this.$utils.msg('审核成功');
