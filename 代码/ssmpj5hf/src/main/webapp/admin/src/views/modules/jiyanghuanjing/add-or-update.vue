<template>
  <div class="addEdit-block">
    ...
    >
      <el-row >
      <el-col :span="12">
        <el-form-item class="input" v-if="type!='info'"  label="区域名称" prop="quyumingcheng">
          <el-input v-model="ruleForm.quyumingcheng" 
              placeholder="区域名称" clearable  :readonly="ro.quyumingcheng"></el-input>
        </el-form-item>
        <div v-else>
          <el-form-item class="input" label="区域名称" prop="quyumingcheng">
              <el-input v-model="ruleForm.quyumingcheng" 
                placeholder="区域名称" readonly></el-input>
          </el-form-item>
        </div>
      </el-col>
      <el-col :span="12">
      ...
      <el-col :span="12">
        <el-form-item class="input" v-if="type!='info'"  label="消毒次数" prop="xiaoducishu">
          <el-input v-model="ruleForm.xiaoducishu" 
              placeholder="消毒次数" clearable  :readonly="ro.xiaoducishu"></el-input>
        </el-form-item>
        <div v-else>
          <el-form-item class="input" label="消毒次数" prop="xiaoducishu">
              <el-input v-model="ruleForm.xiaoducishu" 
                placeholder="消毒次数" readonly></el-input>
          </el-form-item>
        </div>
      </el-col>
      <el-col :span="12">
      ...
      <el-col :span="24">  
        <el-form-item class="upload" v-if="type!='info' && !ro.huanjing" label="环境" prop="huanjing">
          <file-upload
          tip="点击上传环境"
          action="file/upload"
          :limit="3"
          :multiple="true"
          :fileUrls="ruleForm.huanjing?ruleForm.huanjing:''"
          @change="huanjingUploadChange"
          ></file-upload>
        </el-form-item>
        <div v-else>
          <el-form-item v-if="ruleForm.huanjing" label="环境" prop="huanjing">
            <img style="margin-right:20px;" v-bind:key="index" v-for="(item,index) in ruleForm.huanjing.split(',')" :src="$base.url+item" width="100" height="100">
          </el-form-item>
        </div>
      </el-col>
      </el-row>
  ...
  methods: {
    ...
    init(id,type) {
      ...
      if(this.type=='info'||this.type=='else'){
        ...
        for (var o in obj){
          if(o=='quyumingcheng'){
            this.ruleForm.quyumingcheng = obj[o];
	    this.ro.quyumingcheng = true;
            continue;
          }
          if(o=='tuoguanfeiyong'){
	    ...
	    this.ro.tuoguanfeiyong = true;
          ...
          if(o=='xiaoducishu'){
            this.ruleForm.xiaoducishu = obj[o];
	    this.ro.xiaoducishu = true;
            continue;
          }
          if(o=='mianji'){
	...
	this.ruleForm.xiangqing = this.ruleForm.xiangqing.replace(reg,'../../../ssmpj5hf/upload');
        } else {
          this.$message.error(data.msg);
        }
      });
	...
	if(this.ruleForm.huanjing!=null) {
		this.ruleForm.huanjing = this.ruleForm.huanjing.replace(new RegExp(this.$base.url,"g"),"");
	}
...
var objcross = this.$storage.getObj('crossObj');
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `jiyanghuanjing/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.jiyanghuanjingCrossAddOrUpdateFlag = false;
						 this.parent.search();
    ...
    huanjingUploadChange(fileUrls) {
	this.ruleForm.huanjing = fileUrls;
	this.addEditUploadStyleChange()
    },
	addEditStyleChange() {
	  this.$nextTick(()=>{
